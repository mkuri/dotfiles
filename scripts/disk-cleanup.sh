...
TARGETS=(
  ...
  "npm cache|$HOME/.npm|auto-safe|command -v npm|npm cache clean --force|npm re-downloads packages into this cache on demand."
  "Go module cache|$HOME/go/pkg/mod|auto-safe|command -v go|go clean -modcache|Go re-downloads modules into this cache on demand."
  "uv cache|$HOME/.cache/uv|auto-safe|command -v uv|uv cache clean|uv re-downloads packages into this cache on demand."
  "Gradle dependency cache|$HOME/.gradle/caches|auto-safe||rm -rf \"$HOME/.gradle/caches\"/*|Gradle re-downloads dependencies into this cache on demand; Gradle's own docs say deleting this directory is safe."
  "Deno cache|$HOME/.cache/deno|auto-safe||rm -rf \"$HOME/.cache/deno\"/*|Deno re-downloads modules into this cache on demand."
