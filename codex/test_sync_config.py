...
MANAGED_CONFIG = Path(__file__).with_name("managed-config.toml")
...
""".lstrip(),
            ...
            changed, _ = sync_config.synchronize(
                target, MANAGED_CONFIG, apply=True
            )
    ...
    def test_second_apply_does_not_rewrite_file(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            target = Path(directory) / "config.toml"
            sync_config.synchronize(target, MANAGED_CONFIG, apply=True)
    ...
    def test_check_detects_drift_without_writing(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            changed, message = sync_config.synchronize(
                target, MANAGED_CONFIG, apply=False
            )
